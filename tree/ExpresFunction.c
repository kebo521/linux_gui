...
{//Tag,Data
	...
	base64_decode(pOut,&OutLen,pInPar->pStr,pInPar->Len);
	//--------填充结果------------------------
...
{//Tag,Data
	...
	OutLen=pInPar->Len*2;
	pOut=(char*)malloc(OutLen);
	base64_encode(pOut,&OutLen,pInPar->pStr,pInPar->Len);
	//--------填充结果------------------------
	free(pInPar->pStr);
	pInPar->pStr=pOut;
	pInPar->Len=OutLen;
...
{//Tag,Data
	...
	OutLen=pInPar->Len/2;
	pOut=(char*)malloc(OutLen);
	pInPar->Len=APP_Conv_StrToBcd(pInPar->pBuff,pInPar->Len,pOut);
	free(pInPar->pStr);
	pInPar->pBuff=pOut;
	//--------填充结果------------------------
	pInPar->type=DATA_DEFINE_BUFF;
...
{//Tag,Data
	...
	if (err)
	{
		TRACE("打开[%s]文件[%d]失败", pInPar->pStr, err);
	}
	else
	{
...
{//Tag,Data
	...
	if((KeyLen%8)==0)
	{
		APP_Conv_StrToBcd(pKey->pBuff,pKey->Len,Key);
		if(KeyLen==8)
		{
...
{//Tag,Data
	...
	APP_Conv_StrToBcd(pKey->pBuff,pKey->Len,Sm4Key);
	KeyLen=pKey->Len/2;
...
void gDynamicPasswordCheck(u8* sRand,u8 ComIndex,u8 BaudIndex,u16 OperatorIndex,u16 DebugTime,char* pOutData)
{
	...
	pOutData[sLen]='\0';
	TRACE_HEX("pOutData",pOutData,23);
}
...
{//Tag,Data
	...
	strcpy_s(pInPar->pStr,strlen(OutStr+1)+2,OutStr);
	return pInPar;		
}
...
EXP_UNIT* EXP_GetLotteryBit(EXP_UNIT* pInPar)
{
	...
	if(pInPar)
	{
		...
		sprintf_s(pInPar->pStr,128,"Data[%d]total[%03d]\n",pInDataLink->Data,pInDataLink->total);
	...
	else
	{
		...
		while(MaxLink--)
		{
			sprintf_s(pBit,64,"Data[%d]total[%03d]\n",pInDataLink->Data,pInDataLink->total);
