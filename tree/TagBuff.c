...
char* APP_BitXorBuff_New(char* pBitData,u32 BitLen,u32 StartBit,u32 BitType)
{
	...
	char *pBit,*pEnd,*pOut,OutBuff[4096];
	bit2=BitType&0xff;
	BitType >>= 8;
	bit1=BitType&0xff;
	BitType >>= 8;
	...
	pOut=OutBuff;
	pBit=pBitData+StartBit;
	pEnd=pBitData+BitLen;
...
void APP_fileSave(char* pFileName,void *pInBuff,int Inlen)
{
	...
	FILE *f;
	errno_t err;	
	err = fopen_s(&f, pFileName, "wb"); //(const char*)name
	if (err)
	{
		TRACE("打开[%s]文件[%d]失败",pFileName, err);
		return ;
	}
...
void* APP_fileRead(char* pFileName,int *pOutLen)
{
	...
	errno_t err;
	char *pBuff;
	err = fopen_s(&f, pFileName, "rb"); //(const char*)name
	if (err)
	{
		TRACE("打开[%s]文件[%d]失败", pFileName, err);
		return NULL;
